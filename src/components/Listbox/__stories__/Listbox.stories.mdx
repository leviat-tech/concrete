...
export const componentTemplate = (args) => ({
  ...
  template: '<CListbox class="w-64" v-bind="args" v-model="value"></CListbox>',
});
...
const value = [];
const options = ['Listbox', 'Test', 'Values', 'Storybook'];
<CListbox v-model="value" :options="options" :multiple="true"></CListbox>
`}
/>
...
<CListbox v-model="value" :options="options" :formatter="formatter"></CListbox>
`}
/>